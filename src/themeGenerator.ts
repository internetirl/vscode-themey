...
export interface IThemeGenerator {
    GenerateTheme(themeName: string, colorPalette: ColorPalette, outputFile: string): void;
}

export class VibrantDarkThemeGenerator implements IThemeGenerator {
    GenerateTheme(themeName: string, colorPalette: ColorPalette, outputFile: string) {
        const defaultThemeName = themeName;

        let colorSet: IColorSet = {
            base: {
                background: '#1e1e1e',
                foreground: colorPalette.LightMuted,
                color1: colorPalette.Muted,
                color2: colorPalette.Vibrant,
                color3: colorPalette.Vibrant,
                color4: colorPalette.LightVibrant
            }
        };
        generateTheme(defaultThemeName, colorSet, outputFile);
    }
}

export class VibrantDarkMutedThemeGenerator implements IThemeGenerator {
    GenerateTheme(themeName: string, colorPalette: ColorPalette, outputFile: string) {
        const altThemeName = themeName;

        let colorSet: IColorSet = {
            base: {
                background: colorPalette.DarkMuted,
                foreground: colorPalette.LightMuted,
                color1: colorPalette.Muted,
                color2: colorPalette.Vibrant,
                color3: colorPalette.Muted,
                color4: colorPalette.Vibrant
            }
        };
        generateTheme(altThemeName, colorSet, outputFile);
    }
}

export class Base16ThemeGenerator implements IThemeGenerator {
    GenerateTheme(themeName: string, colorPalette: ColorPalette, outputFile: string) {
        ...
        Object.keys(colorPalette).forEach((key) => {
            colorPalette[key] = colorPalette[key].replace('#', '');
        });

        // Read in the default values for Base0 -> Base7
        let templateValues = JSON.parse(fs.readFileSync(path.resolve(themesDir, templateValuesFile), 'utf-8'));
        // Read in the theme template (skipping the comments in the file)
        let templateFile = cjson(fs.readFileSync(path.resolve(themesDir, themeTemplateFile), 'utf-8'));

        // These are for the VSCode interface. Let's keep a default 
        // colorPalette for VSCode's UI (black->white) and build ontop of it.
        ...
        let offVibrant = ColorHelper.shadeColor(colorPalette.Vibrant, 0.5);

        // Variables, XML Tags, Markup Link Text, Markup Lists, Diff Deleted
        templateValues["base08-hex"] = colorPalette.Vibrant;
        //  Integers, Boolean, Constants, XML Attributes, Markup Link Url
        templateValues["base09-hex"] = colorPalette.LightMuted;
        //  Classes, Markup Bold, Search Text Background
        templateValues["base0A-hex"] = offVibrant;
        // Strings, Inherited Class, Markup Code, Diff Inserted
        templateValues["base0B-hex"] = colorPalette.LightVibrant;
        // Support, Regular Expressions, Escape Characters, Markup Quotes
        templateValues["base0C-hex"] = colorPalette.Vibrant;
        //  Functions, Methods, Attribute IDs, Headings
        templateValues["base0D-hex"] = colorPalette.Muted;
        //  Keywords, Storage, Selector, Markup Italic, Diff Changed
        templateValues["base0E-hex"] = colorPalette.Vibrant;
